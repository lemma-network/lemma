...
fn exhaust(rl: &mut RateLimiter, account: &Address, now: Instant) {
    for _ in 0..5 {
        rl.try_acquire(account, now).expect("should not be rate-limited during burst");
    }
}
...
fn burst_exhausted_on_sixth_acquire() {
    ...
    assert!(matches!(err, MempoolError::RateLimited { .. }), "unexpected: {err}");
}
...
fn rate_limited_retry_after_ms_is_positive() {
    ...
    match rl.try_acquire(&a, now).expect_err("must be rate limited") {
        MempoolError::RateLimited { retry_after_ms, .. } => {
            assert!(retry_after_ms > 0, "retry_after_ms must be > 0, got {retry_after_ms}");
        }
...
fn rate_limited_retry_after_ms_is_sane() {
    ...
    match rl.try_acquire(&a, now).expect_err("must be rate limited") {
        MempoolError::RateLimited { retry_after_ms, .. } => {
            ...
            assert_eq!(retry_after_ms, 100, "expected 100ms wait, got {retry_after_ms}");
        }
        other => panic!("unexpected error: {other}"),
    }
}
...
fn acquire_permitted_exactly_at_one_token_refill_point() {
    ...
    assert!(rl.try_acquire(&a, later).is_ok(), "1 token must be available after 100ms");
    assert!(
...
fn partial_refill_permits_correct_number_of_acquires() {
    ...
    assert!(rl.try_acquire(&a, later).is_ok(), "1st acquire on 2.5 tokens must succeed");
    assert!(rl.try_acquire(&a, later).is_ok(), "2nd acquire on 2.5 tokens must succeed");
    assert!(
        rl.try_acquire(&a, later).is_err(),
...
fn prune_removes_full_keeps_limited() {
    ...
    assert_eq!(rl.tracked_accounts(), 0, "both buckets full after 1s → both pruned");
}
...
fn tracked_accounts_same_account_does_not_increment() {
    ...
    rl.try_acquire(&a, now).ok();
    assert_eq!(rl.tracked_accounts(), 1, "same account must not add a second bucket");
}
