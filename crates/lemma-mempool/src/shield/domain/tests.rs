...
fn share_ids_never_contain_zero() {
    ...
    for w in [1u64, 4, 10, 100] {
        let d = ShieldDomain::new(w).unwrap();
        assert!(!d.share_ids().contains(&0), "W={w}: share_ids must not contain 0");
    }
}
...
fn lambda_full_length_equals_share_count() {
    ...
    for id in 1u16..=10 {
        assert!(d.lambda_at_full(id).is_some(), "lambda_at_full({id}) should be Some");
    }
}
...
fn lambda_at_full_out_of_range_returns_none() {
    ...
    assert!(d.lambda_at_full(11).is_none(), "share_id=11 > W=10 is out of range");
    assert!(d.lambda_at_full(u16::MAX).is_none(), "share_id=u16::MAX > W=10 is out of range");
}
...
fn lambda_full_sum_is_one_for_complete_set() {
    ...
    let sum: Fr = (1u16..=5).filter_map(|id| d.lambda_at_full(id)).fold(Fr::zero(), |acc, x| acc + x);
    assert_eq!(sum, Fr::one(), "Σ λ_k(0) over complete set must equal 1 in 𝔽_r");
}
...
fn same_w_produces_identical_share_ids() {
    ...
    assert_eq!(d1.share_ids(), d2.share_ids(), "share_ids must be deterministic");
}
