//! Fuzz target for the Lem safety analyzer (SAFETY-001…025).
//!
//! # Purpose
//!
//! Ensures the safety analyzer NEVER PANICS on arbitrary input, regardless of
...
//! # Strategy
//!
//! Fuzz at the **tokenize → parse → check pipeline level**.  After pipeline
//! wiring (P3·Step 4g), `lemma_lang::check()` internally runs WF + analyze_safety.
//! If the input is valid enough to survive tokenize + parse + type-check + WF,
//! `analyze_safety` will run on it.  If any earlier stage returns `Err(_)`,
//! that is fine — only panics are failures.
//!
...
//!    tokenize/parse/check/analyze_safety is a bug.
//! 3. The fuzzer will naturally discover inputs that reach the safety pass
...
//! cd lemma/crates/lemma-lang && cargo fuzz build fuzz_safety
//!
//! # Run with a time limit (always use a timeout — never run unbounded):
//! cd lemma/crates/lemma-lang && timeout 60 cargo fuzz run fuzz_safety -- -max_total_time=55
//! ```
//!
//! # Crash handling
//!
//! If the fuzzer finds a crash, it writes the input to
//! `fuzz/artifacts/fuzz_safety/crash-*`.  Report the crash input to the
...
fuzz_target!(|data: &[u8]| {
    ...
    // Stage 3: type-check + WF + analyze_safety (pipeline wiring 4g).
    // After 4g, check() internally runs WF + analyze_safety for every contract.
